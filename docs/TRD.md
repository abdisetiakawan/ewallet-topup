...
Semua endpoint menggunakan prefix `/api/...` dengan format respons `BaseResponse`.
...
| Method | Endpoint                | Deskripsi                 | Auth |
| ------ | ----------------------- | ------------------------- | ---- |
| GET    | `/api/wallet/balance` | Cek saldo user yang login | Yes  |
| POST   | `/api/wallet/topup`   | Top-up saldo              | Yes  |
...
| Method | Endpoint                                            | Deskripsi                              | Auth |
| ------ | --------------------------------------------------- | -------------------------------------- | ---- |
| GET    | `/api/transactions?page=0&size=10&status=SUCCESS` | Riwayat transaksi (pagination, filter) | Yes  |
| GET    | `/api/transactions/{id}`                          | Detail transaksi                       | Yes  |
| POST   | `/api/transactions/pay`                           | Pembayaran ke merchant                 | Yes  |
...
```json
{
  ...
  "data": {
    "transactionId": 11,
    "newBalance": 125000,
    "merchantName": "Toko Buku",
    "status": "SUCCESS"
  }
}
```

*Catatan:* `referenceId` wajib unik; backend harus validasi duplikasi dan beri respons `status: false` jika terjadi.

### 7.4 Error Responses

Selalu mengembalikan `BaseResponse` dengan `status: false` dan `data: null`, contoh:
